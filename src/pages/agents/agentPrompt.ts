...
- media: GET /admin/media?kind=&state=&q=, POST /admin/media/upload-url ({filename, contentType image/png|jpeg|webp|gif|svg+xml, sizeBytes, alt, title}) which answers an UploadTicket {media, uploadUrl, method PUT, headers, expiresAt}; PUT the bytes to uploadUrl sending exactly those headers; then POST /admin/media/{id}/confirm. GET /admin/media/{id}, GET .../usage, PATCH (alt, title), DELETE (never refused for a reference: the API clears every reference first; GET .../impact lists what unlinks). Media referenced from content, sponsors, cookie types, or site settings is by its id.
...
- events: GET/POST /admin/events, GET/PATCH/DELETE /admin/events/{id} (name, year, scheduledAt, wentLiveAt, endedAt, fundsPercent 0 to 100, routeId, routeImageMediaId), POST .../current, POST .../status ({statusId 1 planned|2 scheduled|3 live|4 ended|5 cancelled, notify}; going live requires a healthy active beacon, else 409 no_healthy_beacon), GET .../status-history, GET/POST .../messages ({body 1 to 1000, eventTime, notify}), PATCH/DELETE .../messages/{messageId}, GET .../locations (Accept: text/csv streams every row).
...
- cookie_types: GET/POST /admin/cookie-types ({name, sort, active, icon}), PATCH /admin/cookie-types/{id}, DELETE /admin/cookie-types/{id} (takes the type's cookies with it; GET .../impact says how many; set active false to hide a type without deleting it). Every write answers 409 event_live while an event is live.

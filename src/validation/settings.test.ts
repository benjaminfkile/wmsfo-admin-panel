...
describe("validateSettingValue", () => {
  ...
  it("accepts a decimal on location_min_distance_m", () => {
    const s = specFor("location_min_distance_m")!;
    const r = validateSettingValue(s, "2.5");
    expect(r).toEqual({ ok: true, value: 2.5 });
  });

  it("rejects a decimal on int-only keys", () => {
